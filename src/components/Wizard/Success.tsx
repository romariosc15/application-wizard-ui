...
const Success = ({ stepsLength }: InputProps) => {
    const { currentStep } = useContext(WizardContext);
    return (
        <div className={`px-6 pt-6 ${currentStep > stepsLength ? 'block' : 'hidden'}`}>
            <div className="shadow sm:rounded-md sm:overflow-hidden">
                <div className='px-4 pt-7 pb-10 bg-white space-y-2 text-center'>
                    ...
                    <p>Vamos a analizar tus respuestas y en caso avancemos a una siguiente etapa, nos pondremos en contacto directamente contigo.</p>
                </div>
            </div>
        </div>
    );
